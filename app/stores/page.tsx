...
function CreateStoreModal({ onClose, onCreate }: CreateModalProps) {
  ...
  const handleGenerate = useCallback(() => {
    // Placeholder — real ID/key generated by backend on Create
    setGenerated({ id: "(auto-generated)", key: "(auto-generated)" })
  }, [])
      ...
      .then(data => {
        setStores(
          (data.stores ?? []).map((s: any) => ({
            ...
            txCount: s.transactionCount ?? 0,
            status: !s.isActive ? "Suspended" : (s.transactionCount === 0 ? "Trial" : "Active") as StoreStatus,
            enabled: s.isActive ?? true,
            createdAt: new Date(s.createdAt).toISOString().slice(0, 10),
            lastPing: s.updatedAt ? new Date(s.updatedAt).toLocaleString() : "—",
            successRate: s.transactionCount > 0 ? Math.round(((s.completedCount ?? 0) / s.transactionCount) * 1000) / 10 : 0,
          }))
        )
      })
      .catch(() => {})
