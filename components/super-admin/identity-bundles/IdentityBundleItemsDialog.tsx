...
export function IdentityBundleItemsDialog({
  open, onClose, onItemsChanged, bundleId, bundleName, publicBrandName,
}: Props) {
  const { data, error, isLoading, mutate } = useSWR(
    open ? `/api/admin/identity-bundles/items?bundleId=${bundleId}` : null,
    fetcher
  )
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const res = await fetch("/api/admin/identity-bundles/items", {
        method,
  ...
  async function handleDisable(itemId: string) {
    ...
    try {
      const res = await fetch(`/api/admin/identity-bundles/items?id=${itemId}`, { method: "DELETE" })
      if (res.ok) {
        mutate()
        onItemsChanged()
      }
  ...
  // Descriptor preview
  const previewDescriptor = formData.descriptorName
    ? (publicBrandName ? `${publicBrandName} - ${formData.descriptorName}` : formData.descriptorName)
    : ""
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-8 pb-8 overflow-y-auto">
      ...
      <div
        ...
        data-ui-version="identity-bundle-items-manager-v1"
      >
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#343947] bg-[#1f222c]/60 rounded-t-xl">
          <div>
            <h2 className="text-base font-mono font-semibold text-[#e7edf8]">
              Bundle Items — {bundleName}
            </h2>
            <p className="text-xs text-[#6b7280] mt-0.5 font-mono">
              Manage descriptor products for this identity bundle
            </p>
          </div>
        ...
        <div className="p-6 space-y-5 max-h-[calc(100vh-160px)] overflow-y-auto">
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-[#343947] text-[#97a3b6] text-xs font-mono uppercase tracking-wider">
                    <th className="text-left px-4 py-2.5">Descriptor</th>
                    ...
                    <th className="text-center px-3 py-2.5"><Eye className="w-3 h-3 inline" /></th>
                    <th className="text-center px-3 py-2.5">Sort</th>
                  ...
                  {items.map(item => (
                    <tr key={item.id} className="hover:bg-[#151821]/60 transition-colors">
                      <td className="px-4 py-2.5">
                        ...
                        {item.product_slug && <div className="text-[10px] text-[#6b7280]">/{item.product_slug}</div>}
                      </td>
                      ...
                      <td className="px-3 py-2.5 text-center text-xs font-mono">
                        {item.tracking_expected ? <span className="text-emerald-400">✓</span> : <span className="text-[#6b7280]">—</span>}
                      </td>
                      <td className="px-3 py-2.5 text-center text-xs font-mono text-[#97a3b6]">{item.sort_order}</td>
          ...
          {!isLoading && !error && items.length === 0 && (
            <div className="text-center py-10">
              ...
              <p className="text-sm text-[#6b7280] font-mono">No items in this bundle yet.</p>
              <p className="text-xs text-[#4a5568] font-mono mt-1">Add descriptor products to build the buyer-facing identity.</p>
            </div>
          )}
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input className={INPUT} value={formData.productSlug} onChange={e => set("productSlug", e.target.value)} placeholder="tire-wheel-order" />
                  </div>
                  <div className="md:col-span-2">
                    <label className={LABEL_CLS}>Product Description</label>
                    <input className={INPUT} value={formData.productDescription} onChange={e => set("productDescription", e.target.value)} placeholder="Optional description for shield storefront" />
                  </div>
                  <div>
                    <label className={LABEL_CLS}>Price Min</label>
                    <input type="number" step="0.01" className={INPUT} value={formData.priceMin} onChange={e => set("priceMin", e.target.value)} placeholder="0.00" />
                  </div>
                  <div>
                    <label className={LABEL_CLS}>Price Max</label>
                    <input type="number" step="0.01" className={INPUT} value={formData.priceMax} onChange={e => set("priceMax", e.target.value)} placeholder="0.00" />
                  </div>
                  <div>
                    <label className={LABEL_CLS}>Image URL</label>
                    <input className={INPUT} value={formData.imageUrl} onChange={e => set("imageUrl", e.target.value)} placeholder="https://..." />
                  </div>
                  <div>
                  ...
                  <div className="flex items-center gap-6 pt-4">
                    ...
                    <label className="flex items-center gap-2 text-sm text-[#97a3b6] cursor-pointer">
                      <input type="checkbox" checked={formData.trackingExpected} onChange={e => set("trackingExpected", e.target.checked)} className="accent-emerald-400" />
                      Tracking Expected
                    </label>
                    <label className="flex items-center gap-2 text-sm text-[#97a3b6] cursor-pointer">
                ...
                {/* Descriptor Preview */}
                {previewDescriptor && (
                  <div className="bg-[#0d0f14] border border-[#343947] rounded-lg px-4 py-3">
                    <p className="text-[10px] font-mono text-[#6b7280] uppercase tracking-wider mb-1">PayPal Descriptor Preview</p>
                    <p className="text-sm font-mono text-[#e7edf8]">{previewDescriptor}</p>
                  </div>
                )}
                ...
                {formSuccess && (
                  <div className="flex items-center gap-2 text-sm text-emerald-400 font-mono">
                    <CheckCircle2 className="w-4 h-4" /> {isEdit ? "Item updated" : "Item created"}
                  </div>
                )}
                ...
                <div className="flex items-center justify-end gap-3 pt-1">
                  ...
                  <button type="submit" disabled={saving || formSuccess}
